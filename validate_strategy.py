...
# ─────────────────────────────────────────────────────────────
# M15 data — סטאפ מלא (30 נרות, מותאם ל-n=5)
#
# עם n=5 נדרשים 5 נרות משני הצדדים לאישור swing point.
# לכן swing low מאושר רק בטווח אינדקסים [5, 24] (len=30, n=5).
#
# Phase 1 (bars 0-5):  ירידה לעבר equal low ראשון
# Bar  5: SWING LOW #1  low=1.0850 (window lows[0:11] — min)
# Bars 6-9:  bounce לעבר swing high
# Bar  9: SWING HIGH    high=1.0900 (window highs[4:15] — max)
# Bars 10-15: ירידה חזרה ל-equal low שני
# Bar 15: SWING LOW #2  low=1.0850 (window lows[10:21] — equal lows pool ✓)
# Bars 16-20: אישור bar 15 כ-swing low (כל low > 1.0850)
# Bar 21: SWEEP          low=1.0820 < 1.0850, close=1.0855 > 1.0850 ✓
#          גם SWING LOW (window lows[16:27] — min)
# Bar 22: high=1.0862  ← FVG bottom
# Bar 23: impulse חזק  ← FVG middle
# Bar 24: BOS: close=1.0902 > 1.0900 ✓  low=1.0870 ← FVG top
#          FVG zone=[1.0862, 1.0870]  gap=0.0008 ✓
#          impulse_low=lows[21]=1.0820, impulse_high=1.0900
#          75% entry = 1.0900 - 0.75*0.0080 = 1.0840
# Bar 25: high=1.0912 > highs[24]=1.0905 → bar 24 לא swing high → BOS תקין
# Bars 25-27: pullback, כל low > 1.0870 (FVG top) → FVG לא mitigated
# Bar 28: ENTRY  low=1.0840 = 75% Fib ✓
# ─────────────────────────────────────────────────────────────
M15_ROWS = [
    # open     high     low      close    vol
    (1.0880,  1.0885,  1.0872,  1.0875,  1000),  # 0
    (1.0875,  1.0880,  1.0865,  1.0868,  1000),  # 1
    (1.0868,  1.0872,  1.0858,  1.0862,  1000),  # 2
    (1.0862,  1.0868,  1.0855,  1.0860,  1000),  # 3
    (1.0860,  1.0865,  1.0852,  1.0856,  1000),  # 4
    (1.0856,  1.0860,  1.0850,  1.0858,  1000),  # 5  ← SWING LOW #1: low=1.0850
    (1.0858,  1.0865,  1.0854,  1.0862,  1000),  # 6
    (1.0862,  1.0875,  1.0858,  1.0872,  1000),  # 7
    (1.0872,  1.0890,  1.0868,  1.0885,  1000),  # 8
    (1.0885,  1.0900,  1.0878,  1.0892,  1000),  # 9  ← SWING HIGH: high=1.0900
    (1.0892,  1.0895,  1.0878,  1.0882,  1000),  # 10
    (1.0882,  1.0888,  1.0872,  1.0876,  1000),  # 11
    (1.0876,  1.0882,  1.0865,  1.0870,  1000),  # 12
    (1.0870,  1.0875,  1.0858,  1.0862,  1000),  # 13
    (1.0862,  1.0868,  1.0854,  1.0858,  1000),  # 14
    (1.0858,  1.0862,  1.0850,  1.0856,  1000),  # 15 ← SWING LOW #2: low=1.0850 (equal lows ✓)
    (1.0856,  1.0862,  1.0852,  1.0858,  1000),  # 16 ← אישור bar 15
    (1.0858,  1.0865,  1.0854,  1.0862,  1000),  # 17
    (1.0862,  1.0868,  1.0856,  1.0865,  1000),  # 18
    (1.0865,  1.0870,  1.0858,  1.0868,  1000),  # 19
    (1.0868,  1.0872,  1.0860,  1.0870,  1000),  # 20
    (1.0870,  1.0875,  1.0820,  1.0855,  1000),  # 21 ← SWEEP + SWING LOW: low=1.0820 < 1.0850 ✓
    (1.0855,  1.0862,  1.0848,  1.0858,  1000),  # 22 ← high=1.0862 (FVG bottom)
    (1.0858,  1.0900,  1.0855,  1.0898,  1000),  # 23 ← FVG middle (impulse)
    (1.0898,  1.0905,  1.0870,  1.0902,  1000),  # 24 ← BOS: close=1.0902>1.0900 ✓ | FVG top: low=1.0870
    (1.0902,  1.0912,  1.0882,  1.0888,  1000),  # 25 ← high=1.0912>1.0905 → bar 24 לא swing high
    (1.0888,  1.0895,  1.0875,  1.0878,  1000),  # 26 ← pullback, low=1.0875 > 1.0870 ✓
    (1.0878,  1.0882,  1.0872,  1.0875,  1000),  # 27 ← low=1.0872 > 1.0870 ✓
    (1.0875,  1.0880,  1.0840,  1.0848,  1000),  # 28 ← ENTRY: low=1.0840 = 75% Fib ✓
    (1.0848,  1.0862,  1.0842,  1.0858,  1000),  # 29 ← bounce מרמת הכניסה
]
...
def run_validation():
    ...
    # בדיקת bias בזמן כניסה (bar 28) — חייב להיות אחרי ה-H1 BOS
    at_time = m15_df.index[28]
    ...
    # mitigation רלוונטי רק מרגע ה-BOS (כמו ב-engine האמיתי — bar-by-bar)
    for j in range(last_bos["bar_idx"] + 1, check_until):
        c = m15_df.iloc[j]
